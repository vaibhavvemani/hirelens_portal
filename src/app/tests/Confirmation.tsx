...
export function countTotalMarks(assessment: Record<string, any>): number {
  let marks = 0
  for (const question in assessment) {
    marks += Number(assessment[question].marks)
  }
  return marks
}

export function marksPerQuestion(assessment: Record<string, any>): number{
  return countTotalMarks(assessment) / countQuestions(assessment)
}
    ...
    <div className="flex flex-col justify-center w-full h-full items-center">
      <Card className="flex flex-col p-3 min-w-[600px] gap-5">
        ...
        <div className="flex w-full justify-between gap-0">
          <Button onClick={onStart} className="w-50/101">
            Start test
          </Button>
