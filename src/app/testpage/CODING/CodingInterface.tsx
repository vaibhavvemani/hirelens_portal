        ...
        <div className="flex gap-4 h-[650px]">
          <Card className="p-3 gap-0 w-1/3 justify-between">
            <div className="flex flex-col gap-3">
              ...
              {question?.image && <Image src={question.image} alt={question.text ?? ''}/>}
              ...
              {question?.questionType === "coding" ? (
                question.sampleResults.map((result, index) => (
                  <div key={index} className="flex flex-col gap-2">
                    ...
                    <div className="flex flex-col gap-1 ml-1 pl-2 border-l-4">
          ...
          <CodingIDE languages = {question?.questionType==="coding"? question?.languages?? SUPPORTED_LANGUAGES : SUPPORTED_LANGUAGES} />
        </div>
      </div>
    </div>
  );
